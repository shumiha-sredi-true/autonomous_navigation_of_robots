import asyncio
import websockets  # Убедитесь, что импортирована сама библиотека
import websockets.protocol  # Для доступа к State
import ssl
import certifi
import json
import cv2
...
async def detect_and_send_apriltags(websocket):
    ...
    try:
        # Используем websocket.state для проверки
        while websocket.state == websockets.protocol.State.OPEN:
                    ...
                    try:
                        ...
                        if ret_pnp:
                            ...
                            if tag_id_raw == CALIBRATION_TAG_ID:
                                ...
                                print(tags_data_for_current_frame[-1])
            ...
            if current_time_mono - last_send_time >= SEND_INTERVAL:
                # Повторная проверка состояния перед отправкой
                ...
                try:
                    ...
                    # if len(tags_data_for_current_frame) > 0:
                    #     pass # print(...)
...
async def receive_updates(websocket):
    ...
    try:
        ...
        pass  # Уже обработано выше или будет обработано в main_client
    except asyncio.CancelledError:
...
async def main_client():
    ...
    while True:
        active_websocket = None  # Переменная для хранения активного объекта websocket
        tasks = []
        try:
            async with websockets.connect(
                SERVER_URI, ssl=ssl_context, ping_interval=20, ping_timeout=10
            ) as websocket:
                active_websocket = websocket  # Сохраняем для finally
                ...
                if auth_response.get("status") != "success":
                    ...
                    # Вместо raise, просто дадим циклу while True переподключиться после sleep в finally
                    # Это позволит избежать краха, если сервер временно отклоняет авторизацию
                    # Пометим, что нужно переподключиться без нормального завершения сессии
                ...
                # Логика завершения и отмены задач
                # (остается такой же, как в предыдущей версии, так как она корректна)
                print(
        ...
        except asyncio.TimeoutError:  # Таймаут на websocket.recv() при авторизации
        ...
        ) as e:  # Наше кастомное для управления переподключением
            ...
            # Отменяем задачи, если они еще активны (на случай выхода из try блока до asyncio.wait)
            ...
            if tasks:  # Ждем завершения отмены
                await asyncio.gather(*tasks, return_exceptions=True)

            # `async with` автоматически закроет соединение `websocket`.
            # Явное закрытие `active_websocket.close()` не всегда нужно,
            # но может быть полезно, если выход из `async with` не произошел штатно.
            # Однако, если `active_websocket` это тот же `websocket` из `async with`,
            # то повторное закрытие может вызвать ошибку.
            # Лучше положиться на `async with`.
            # if active_websocket and active_websocket.state != websockets.protocol.State.CLOSED:
            #     print(f"[{datetime.now()}] Закрытие WebSocket соединения в finally (если еще не закрыто)...")
